...
/*   Updated: 2022/01/24 10:47:06 by lbattest         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	error(int i)
{
	if (i == 1)
		...
		write(1, "Problem when executing the function", 35);
	exit(1);
}
...
// static int	split_line(char **str, char **argv, int i, char c)
// {
// 	int	j;
...
int	main(int argc, char **argv, char **envp)
{
	...
	else // wc || parent
	{
		...
		free(cmd[0]);
		cmd[0] = path;
		fd = open(argv[4], O_CREAT | O_TRUNC | O_RDWR);
		if (fd == -1)
			error(5);
		close(1);
		dup2(fd, 1);
